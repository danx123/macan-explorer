...
from PyQt6.QtCore import (
    Qt, QSize, pyqtSignal, QDir, QUrl, QFileInfo, QSortFilterProxyModel,
    QModelIndex, QPoint, QRect, QEvent, QTimer, QMimeData
)
from PyQt6.QtGui import (
    QAction, QIcon, QActionGroup, QDesktopServices, QFileSystemModel, QPixmap,
...
class ThumbnailIconProvider(QFileSystemModel):
    ...
    def _generate_video_thumbnail(self, video_path, thumbnail_path):
        try:
            command = [
                'ffmpeg',
                '-i', video_path,
            ...
            self.logger.log(f"Failed to generate thumbnail for {os.path.basename(video_path)}: {e}", level="warning", tag="FFMPEG")
            return False       
    ...
    def data(self, index, role):
        if role == Qt.ItemDataRole.DecorationRole and index.column() == 0:
            ...
            # Video handler with FFmpeg
            if FFMPEG_AVAILABLE and suffix in self.VIDEO_EXTENSIONS:
                thumbnail_path = self._get_cached_thumbnail_path(file_path)
                
                # Check for a valid cached thumbnail first
                try:
                    ...
                    # The source file might have been deleted during the check
                    pass
...
class FileView(QWidget):
    ...
    status_message_requested = pyqtSignal(str) # For copy/paste status updates
    ...
    def paste_items(self):
        ...
        self.status_message_requested.emit(f"Pasting {len(urls)} item(s)...")
        QApplication.processEvents() # Update UI to show message

        for url in urls:
            if url.isLocalFile():
                src_path = url.toLocalFile()
                dest_path_base = os.path.join(dest_dir, os.path.basename(src_path))
                ...
                if os.path.isdir(src_path) and os.path.normpath(dest_path).startswith(os.path.normpath(src_path)):
                    QMessageBox.warning(self, "Paste Error", f"Cannot copy '{os.path.basename(src_path)}' into a subdirectory of itself.")
                    continue
                ...
                if os.path.exists(dest_path):
                    ...
                    while os.path.exists(dest_path):
                        dest_path = os.path.join(dest_dir, f"{base} - copy ({i}){ext}")
                        i += 1
                ...
                try:
                    ...
                    QMessageBox.critical(self, "Paste Error", f"Failed to paste '{os.path.basename(src_path)}':\n{e}")
    ...
    def delete_selected_items(self):
        ...
        reply = QMessageBox.question(self, 'Confirm Deletion',
                                     f"Are you sure you want to permanently delete {len(paths_to_delete)} selected item(s)?",
...
def check_for_ffmpeg():
    """Checks if FFmpeg is in the system's PATH and sets the global flag."""
    ...
    try:
        ...
        logging.info("FFmpeg executable found. Video thumbnails are enabled.")
    except (FileNotFoundError, subprocess.CalledProcessError):
        FFMPEG_AVAILABLE = False
        logging.warning("FFmpeg executable not found in PATH. Video thumbnails will be disabled.")
        logging.warning("To enable video thumbnails, please install FFmpeg and ensure it is in your system's PATH.")
