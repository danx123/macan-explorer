...
SVG_ICONS = {
    ...
    # --- MODIFIKASI START: Ikon SVG 'refresh' diganti ---
...
/* --- MODIFIKASI START: Style untuk label permanen di status bar --- */
QStatusBar QLabel {
    color: #e0e0e0;
    padding: 0 5px;
}
...
class FileView(QWidget):
    ...
    # --- MODIFIKASI START: Signal baru untuk info status bar ---
    ...
    def __init__(self, config_manager, folder_path=None, parent=None):
        ...
        # --- MODIFIKASI START: Aktifkan drop di widget FileView ---
        ...
        # --- MODIFIKASI START: Aktifkan drag (hanya drag) di view ---
        ...
        # --- MODIFIKASI START: Hubungkan sinyal perubahan seleksi ---
    ...
    def set_path(self, path, add_to_history=True, initial_load=False):
        ...
        # --- MODIFIKASI START: Perbarui status bar saat path berubah ---
    ...
    # --- MODIFIKASI START: Handler untuk perubahan seleksi ---
        ...
        try:
            ...
            if num_selected == 0:
                ...
                if info.isDir():
                    ...
                    size_bytes = _get_folder_size(path) # Ini bisa lambat untuk folder besar
                else:
                    type_desc = f"{info.suffix().upper()} File" if info.suffix() else "File"
                    size_bytes = info.size()
                
                size_str = _format_size(size_bytes)
                self.selection_info_requested.emit(f"1 item selected: {name}  |  Type: {type_desc}  |  Size: {size_str}")
                ...
                for path in paths:
                    info = QFileInfo(path)
                    if info.isDir():
                        total_size += _get_folder_size(path)
                    else:
                        total_size += info.size()
                
                size_str = _format_size(total_size)
                self.selection_info_requested.emit(f"{num_selected} items selected  |  Total Size: {size_str}")
        except Exception as e:
            # Tangani error jika view belum siap
    ...
    # --- MODIFIKASI START: Logika paste dipindah ke helper ---
    def paste_items(self):
        ...
        urls = mime_data.urls()
        self._perform_paste_operation(urls)
        
    def _perform_paste_operation(self, urls):
        """Helper function to perform the copy/paste operation from a list of QUrls."""
    ...
    # --- MODIFIKASI START: Event handler untuk Drag and Drop ---
    ...
    def dragMoveEvent(self, event):
        """Accepts the move event during drag."""
        event.acceptProposedAction()

    def dropEvent(self, event):
        """Handles the drop event by pasting the files."""
        if event.mimeData().hasUrls():
            urls = event.mimeData().urls()
            self._perform_paste_operation(urls) # Gunakan helper yang sama dengan paste
            event.acceptProposedAction()
    # --- MODIFIKASI END ---
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # --- MODIFIKASI START: Setup status bar permanen ---
        self.status_label = QLabel("🛕 Shrine ready to explore...")
        self.status.addPermanentWidget(self.status_label, 1) # Stretch factor 1
        # --- MODIFIKASI END ---
    ...
    def connect_active_tab_signals(self, tab_widget):
        if not isinstance(tab_widget, FileView):
            ...
            self.update_permanent_status("") # Kosongkan status bar
            return
        ...
        try:
            if hasattr(self, '_active_tab_widget') and self._active_tab_widget:
                ...
                # --- MODIFIKASI START: Disconnect sinyal status bar ---
        ...
        # --- MODIFIKASI START: Connect sinyal status bar ---
        ...
        # --- MODIFIKASI START: Inisialisasi status bar untuk tab baru ---
    ...
    # --- MODIFIKASI START: Method baru untuk status permanen ---
    def update_permanent_status(self, message):
        """Sets the permanent message on the status bar."""
        self.status_label.setText(message)
    # --- MODIFIKASI END ---

    # --- FIX START: COPY/PASTE STATUS ---
    def update_status_bar(self, message):
        """Shows a temporary message on the status bar for a few seconds."""
